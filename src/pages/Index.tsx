...
const Index = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-orange-50 via-yellow-50 to-red-50">
      ...
      <header className="bg-gradient-to-r from-orange-600 via-red-600 to-pink-600 text-white p-4 sticky top-0 z-10 shadow-lg">
      ...
      {showSpecialOffer && (
        <div className="bg-gradient-to-r from-purple-600 to-blue-600 text-white p-4 mx-4 mt-4 rounded-xl shadow-lg animate-slide-in-right">
          <div className="flex items-center justify-center space-x-2">
            <Sparkles className="h-5 w-5 animate-spin" />
            <span className="font-bold text-lg">⚡ Limited Time Offer!</span>
            <Sparkles className="h-5 w-5 animate-spin" />
          </div>
          <p className="text-center text-sm mt-1">Free delivery on orders above ₹100! 🚀</p>
        </div>
      )}

      {/* Coupon Banner */}
      {couponApplied && (
        <div className="bg-gradient-to-r from-green-500 to-emerald-600 text-white p-4 mx-4 mt-4 rounded-xl shadow-lg animate-scale-in">
          <div className="flex items-center justify-center space-x-2">
            <Gift className="h-6 w-6 animate-bounce" />
            <span className="font-bold text-lg">🎉 Congratulations!</span>
          </div>
          <p className="text-center text-sm mt-1 flex items-center justify-center space-x-1">
      ...
      <div className="p-4 space-y-4">
        <div className="flex items-center space-x-2 mb-6">
          <h2 className="text-2xl font-bold text-gray-800">Order Fresh Biryani</h2>
        ...
        {menuItems.map((item) => (
          <div 
            key={item.id} 
            className={`bg-white rounded-xl shadow-lg p-6 hover:shadow-xl transition-all duration-300 hover:scale-[1.02] border border-gray-100 ${
              pulsingItems.has(item.id) ? 'animate-pulse bg-green-50' : ''
            }`}
          >
            <div className="flex items-center space-x-4">
              <div className="text-5xl animate-pulse">{item.image}</div>
              ...
              <div className="flex items-center space-x-2">
                {getItemQuantity(item.id) > 0 ? (
                  <div className="flex items-center space-x-3 bg-orange-50 rounded-full p-1">
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => handleRemoveFromCart(item.id)}
                      className="h-10 w-10 p-0 rounded-full border-orange-200 hover:bg-orange-100 hover:border-orange-300 transition-all duration-200"
                    >
                    ...
                    <Button
                      ...
                      className="h-10 w-10 p-0 rounded-full bg-orange-500 hover:bg-orange-600 transition-all duration-200 transform hover:scale-110"
                    >
        ...
        {!couponApplied && totalItems > 0 && (
          <div className="bg-gradient-to-r from-yellow-50 to-orange-50 border border-yellow-200 rounded-xl p-4 mt-6 animate-fade-in">
      ...
      {totalItems > 0 && (
        <div className="fixed bottom-0 left-0 right-0 bg-white border-t shadow-2xl p-4 animate-slide-in-right">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-2xl font-bold text-gray-800">Total: ₹{totalAmount}</p>
              {couponApplied && (
          ...
          <Button
            onClick={handleNext}
            className="w-full bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700 text-white py-4 text-lg font-bold rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-[1.02]"
          >
